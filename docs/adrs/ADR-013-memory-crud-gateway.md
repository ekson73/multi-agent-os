...
- **E6 / mutation-hook (§"E6")** — `update` resolves supersession *in the same operation*: writing a fact that obsoletes an existing topic archives the superseded one and updates the index atomically, with the supersession recorded in the ledger. Refusing to guess is a valid outcome — an ambiguous supersession **refuses with a pointer** (same shape as `DEDUP_COLLISION`) rather than archiving the wrong topic; silent non-archival is not acceptable, silent wrong-archival is worse.
...
**Amendment 2026-07-23 — §"E6" + 2 DoD items (mutation-hook forgetting).** Adds the E6 evaluation lens (*judge a memory mechanism by what it can FORGET, not only by what it can find*) and the two DoD items it implies. Net-new to this ADR (DRY-checked: zero prior mentions of E6 / mutation-hook / intent-aware / forget). Empirical basis: arXiv 2606.15903 — deterministic ≤5% canonicalization / 0% cross-lingual · inscribe-time LLM 100% canonicalization but **0% intent-aware deletion** · +mutation-time hook **+22.6–24.1 pt** (70.1→93.3%). The gap this closes is structural, not cosmetic: all four mutating verbs are caller-initiated, so the original DoD could be met in full by a gateway that never forgets anything on its own. Companion decision: akasha `docs/decisions/ADR-memory-placement-router-2026-07-22` (the router classifies `control_plane` regime per item; this gateway executes it) + `memory-architecture-roadmap-2026-07-15` §4/§6 Phase-3b. Tracked: EKO-116. Agent-authored (Claude Opus 4.8, session `318de395`) under the operator's standing continue-directive; `creator` on the tracker = MCP-auth account (operator), not the author.
