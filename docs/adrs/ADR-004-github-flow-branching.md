...
6. Because PRs are squash-merged, a version delta MUST use a separate, rebased PR containing exactly one `chore(release): ...` commit. Repository squash settings are `COMMIT_OR_PR_TITLE + COMMIT_MESSAGES`, preserving that one commit subject on `main`. The release artifact contains only the monotonic `plugin.json` version field plus one matching additive changelog section. README/CLAUDE versions are derived post-merge; feature code and release metadata do not share a PR. The trusted-base workflow treats the head only as Git data and emits a commit-invariant check. That check remains advisory at platform level until its verdict can be bound to a dedicated publisher identity or an organization-level required workflow; the shared GitHub Actions identity is not accepted as proof of workflow origin.
