...
# The predicate itself lives in scripts/entry-classifier.sh so the CI CHANGELOG gate
# (scripts/changelog-required-check.sh, wired by .github/workflows/changelog-required.yml)
# classifies with the SAME definition instead of a second, drifting regex — #348 measured
# what that second copy costs: it missed 30 nested entries and swallowed 4 ALL-CAPS docs.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=../scripts/entry-classifier.sh
. "$SCRIPT_DIR/../scripts/entry-classifier.sh"
