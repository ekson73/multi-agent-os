...
SERVER_INFO = {
    ...
    "version": "2.0.0",
    "description": "Monitor, trigger, and analyze Bitbucket Cloud pipelines with AI-powered diagnosis, auto-learning, deployments, variables, caches, commits, PRs, and branch management. Supports Bearer token auth and multi-repo.",
    "mcp_server": "hub.py",
    "env_vars": {
        "BITBUCKET_USERNAME": "Bitbucket username (optional if using Bearer token)",
        "BITBUCKET_APP_PASSWORD": "App password or Bearer token (ATCTT3x prefix = auto-Bearer)",
        "BITBUCKET_REPO_SLUG": "(Optional) Override repo slug (workspace/repo-name)",
        "BITBUCKET_AUTH_TYPE": "(Optional) Force auth type: 'bearer' or 'basic' (auto-detected from token prefix)",
