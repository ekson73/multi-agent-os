#!/bin/bash
while true; do
  OUTPUT=$(python3 cli.py bitbucket get_build_details "{\"build_number\": 1011, \"workspace\": \"vek-servicos\", \"repo_slug\": \"vks-jss-sales-api\"}")
  ...
  if [[ "$STATE" == "COMPLETED" ]]; then
    if [[ "$RESULT" == "SUCCESSFUL" ]]; then
      ...
      python3 cli.py bitbucket merge_pull_request '{"pr_id": 3, "workspace": "vek-servicos", "repo_slug": "vks-jss-sales-api"}'
      break
    else
