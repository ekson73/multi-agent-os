...
| Cross-check — `target_digest` vs. actual send parameter | independently recomputed `sha256("fake-contact")` and confirmed it equals the plan's `target_digest` used both in the digest and in the literal `--to fake-contact` argument of the real send call — plan-digest integrity alone does not by itself prove the command's actual parameters matched, so this was checked separately. **Promoted to a contract requirement after this run**: `agents/wacli-delegate.md` v0.2.0 Execute gate 6 (parameter binding) now mandates this recomputation for both `target_digest` and `payload_digest` on every execute; `tests/test-wacli-delegate-contract.sh` §3b pins the rule mechanically | PASS |
