...
| 6 | T8-core intake gatekeeper (`IntakeGatekeeper` — triage determinística de candidatos sobre os SSOTs do piso: F1 known-verdict/EXCLUDED+safe-successor · F2 flag-veto T9 · F3 conductor-isolation RULE-011; **never-auto-accept estrutural** — vocabulário fechado sem "accepted"; reject-by-default fail-closed; cerimônia per-PR DEFERRED até existirem PRs inbound) | ADR-007 (Amendment 2026-07-10) | 🟢 merged (2026-07-10) | [#239](https://github.com/ekson73/multi-agent-os/pull/239) |
| 6 | T7 slot-adapter (on-first-vendored-use) — próximo per-tile | ADR-007 (Amendment 2026-07-10) | 🟡 destravada per-tile (gate: consumidor interno nomeado + 7 guardrails + HITL por tile; trigger = primeira integração vendorizada real) | — |
