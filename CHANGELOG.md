...
- **NEW test suite `bin/tests/spawn-continuation-name.test.sh`** (5/5 green, all via `--dry-run` + isolated `CLAUDE_JOBS_DIR`): D1 default-shape · `--status` propagation · legacy env-style · locus-absent fallback (via the `MAOS_LOCUS_BIN` test seam) · dry-run read-only guarantee. Regression: `locus.test.sh` 22/22 + `scorecard-next-model.test.sh` 25/25 stay green.
