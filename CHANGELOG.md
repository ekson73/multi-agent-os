...
- **NEW `docs/adrs/ADR-006-ath-moe-hub-adoption.md`** (Status: **Proposed**) — **proposed decision**: absorb `agentic-moe-2026` as a MAOS **evolution chapter**, and realize the ATH hub as the **MAOS Hub** — the native MoE gating-network, declared **evolution of `maos-mcp-hub`** (not a new product). **Proposes** the **single-conductor invariant** (no co-resident ECC/superpowers/gstack/BASE/ruflo/BMAD — route via `agentic-tool-intake`, reuse patterns DRY, never stack runtimes).
