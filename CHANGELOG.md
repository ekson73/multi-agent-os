...
relay/seed, tier propose/apply e 198 assercoes hermeticas (bash 3.2 + moderno, python, node).
...
- **⛔ Secrets** read from a machine-local env file (or the process environment) — never echoed/logged/committed/placed in argv (the key goes in a chmod-600 `curl --config`). Per-operator keys live in a machine-local `~/.config/eko/speak.env` (allowlist-parsed per `script-safety §2`, never blind-sourced) — the committed file carries **zero** operator credential data; with no config the API engines fall through to **Kokoro** (zero-setup, local). gitleaks-clean.
