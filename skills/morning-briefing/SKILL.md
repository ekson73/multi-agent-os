...
**`--format=json`** emits **one valid JSON document** — a single top-level envelope carrying every measured metric in a `metrics` array (a bare stream of consecutive top-level objects is NOT a valid document: `jq`/`JSON.parse` reject the second object, and the `--audience agent`, `--save` and `--clipboard` paths all consume the whole output as one file). Keys are en-US and versioned with `prompt_version`; each metric object keeps `state` as a first-class field and `bar` as convenience, never source-of-truth:
```json
{
  "prompt_version": "1.9.0",
  "metrics": [
