...
description: |
  Deliver a deterministic, scannable, evidence-backed briefing of operator's current work state across
  ...
  decisions-awaiting · risks · next-action) with optional narrative polish. Also supports end-of-session
  recap mode via `--mode=recap` flag (v1.3.0+) — produces multi-tier objective N-Tree (primary/secondary/
  auxiliary) + tasks/subtasks/steps tree + % done + % PRs green + % agentic convergence + gaps + pendings +
  unasked-questions + unanswered-questions + undecided-decisions + HITL-pendings + Eisenhower 2×2 next-tasks
  + inter-dependencies DAG + blocked/blockers. Use when the operator says "morning briefing", "bom dia,
  retomando", "where was I", "give me a state recap", "what's pending", "session recap", "end of session
...
cycles_evidence:
  ...
  - "2026-05-20 v1.0.1 PATCH — empirical probe identified Claude Code session-name source-of-truth: JSONL type:ai-title .aiTitle (rolling, tail-1) + CLAUDE_CODE_SESSION_ID env var (UUID). Probe latency <100ms validated."
...
i18n:
  ...
  detection_cascade:
    ...
    - "CLAUDE.md or user-rules.md **Language:** line parse"
    - "fallback: en-us"
...
| `--scope=current` (default) | **NEW v1.5.1** (canonical; v1.5.0 used `inside`) — Compass `current()` / `inside()` verb. Render here-and-now state (default behavior; no behavior change when omitted). Maps to `cowork-process-topology-protocol.md` §9. Idiomatic CLI convention (parallel to `pwd`, `kubectl config current-context`, `git status`, `gcloud config get current`). **Session-scope boundary (v1.8.1)**: inspects THIS worktree/session's own state only — never scans sibling worktrees' content (that requires explicit `--scope=sideways`). Prevents silently expanding the briefing's scope into another agentic work-in-progress without operator awareness. |
...
| `--scope=sideways` | **NEW v1.5.0** — Compass `siblings()` verb. Render peers/alternatives at same hierarchical level (worktrees-irmãs, parallel PRs, option matrices A/B/C/D). Pair with `--breadth all\|N` (default `N=5`). **v1.8.1**: this is now the ONLY path that inspects sibling-worktree git status (moved here from the Phase-1 default baseline, which previously did this unconditionally). |
...
| `--multi-repo` | Scan known sibling repo paths beyond cwd (capability-detected) · localized · works in both modes. **Distinct axis from `--scope=sideways`** (v1.8.1 clarification): `--multi-repo` covers DIFFERENT repos entirely; `--scope=sideways` covers SAME-repo sibling worktrees. Neither fires unless explicitly passed. |
...
# Memory location detection (Claude Code convention)
...
# Claude session-name detection (Claude Code; AAIF-portable fallback)
...
# Non-Claude hosts: leave empty; graceful-degrade. NEVER fabricate "unknown" string (anti-theater).
CLAUDE_SESSION_NAME=""
...
#   4. CLAUDE.md OR user-rules.md "**Language**:" line parse
...
detect_lang_code() {
  ...
  if [ -n "${CLI_LANG:-}" ]; then raw="$CLI_LANG";
    ...
    for f in "$HOME/.claude/CLAUDE.md" "$HOME/.claude/rules/user-rules.md"; do
      [ -f "$f" ] || continue
...
detect_aiprovider_root() {
  # Priority: Claude Code first (most common ai-provider), then Cursor, then Aider.
...
# because non-Claude hosts (Cursor/Copilot/Aider) reliably have CLAUDE_CODE_SESSION_ID unset.
...
git worktree list   # enumeration ONLY (paths + count) — cheap metadata for the $worktrees
                     # <n> figure in the Pulse callout. Does NOT `cd` into, nor scan the
                     # content of, sibling worktrees by default — see scope-boundary note.
...
# Claude session-name (already captured in Phase 0 as $CLAUDE_SESSION_NAME)
# No additional probe needed here — passes through to Phase 3 synthesis.
# When empty (non-Claude host OR ai-title absent), header silently omits the field.
```

> **Scope boundary (v1.8.1) — default scope = CURRENT session/worktree only.** Prior versions
> unconditionally `cd`-ed into **every** worktree returned by `git worktree list` and dumped its
> `git status --short` — meaning the default (`--scope=current`) baseline already inspected the
> content of every other agentic work-in-progress sharing this repo (another session's/agent's
> branch), silently contradicting both Phase 2.5's own "`current` = baseline, no expansion" claim
> and anti-pattern #9's "only scan beyond current with explicit flag" principle (a principle whose
> flag, `--multi-repo`, only ever gated *sibling repos* — it never gated this *sibling-worktree*
> loop). Per-worktree content scanning is now an explicit, opt-in expansion under
> `--scope=sideways` (see the `sideways` case in Phase 2.5) — it never fires by default, and it
> never fires silently. This is a fix, not a new capability: it makes the code match what the
> skill already documented it did.

Optional task tool integration (host-specific): if Claude Code TaskList tool available, surface in_progress + blocked tasks.
...
|---|---|---|---|
| **`current`** (default) | Current branch · last 5 commits · 24h status (baseline) · **this worktree/session ONLY — sibling worktrees NOT scanned** (v1.8.1) | MEMORY.md head 100 + 3 most-recent entries | Standard 7-section briefing (no expansion) |
...
case "$SCOPE_VERB" in
  ...
  sideways)
    ...
    # AND sibling-worktree state (moved here from the Phase-1 default baseline in
    # v1.8.1 — scanning another agentic-wip's content is an explicit expansion,
    # never the default; see Phase 1 §"Scope boundary").
    #
    # SCOPE_BREADTH is either a positive integer OR the literal "all" (§ operator-flags
    # table default modifier). `head -N`/`head -"$SCOPE_BREADTH"` is non-portable AND
    # breaks outright when SCOPE_BREADTH=all (`head -all` is an invalid option, not
    # "unlimited"); breadth_limit() below normalizes both: `cat` (unlimited) for "all",
    # `head -n N` (portable form) otherwise. `gh pr list --limit` also requires an
    # integer — "all" maps to a generous ceiling instead.
    if [ "$SCOPE_BREADTH" = "all" ]; then
      breadth_limit() { cat; }
      BREADTH_GH_LIMIT=1000  # gh --limit rejects "all"; ample ceiling stands in for unlimited
    else
      breadth_limit() { head -n "$SCOPE_BREADTH"; }
      BREADTH_GH_LIMIT="$SCOPE_BREADTH"
    fi
    git for-each-ref --format='%(refname:short)' refs/heads/ 2>/dev/null | breadth_limit
    [ "$HAS_GH" = "yes" ] && gh pr list --state open --json number,title,headRefName --limit "$BREADTH_GH_LIMIT"
    # Exclude the CURRENT worktree (per git-worktree(1), the main worktree is listed
    # first — but "current" may itself be a linked worktree, so filter by the actual
    # checked-out toplevel, not by list position) so it never consumes a breadth slot
    # (e.g. --breadth=1 returning zero real siblings). `cut -d' ' -f2-` (not `-f2`) +
    # a line-safe `while read` loop (not `for wt in $(...)`) preserve worktree paths
    # that contain spaces, which shell word-splitting would otherwise truncate.
    CURRENT_WT="$(git rev-parse --show-toplevel 2>/dev/null)"
    git worktree list --porcelain | grep '^worktree' | cut -d' ' -f2- | grep -Fxv "$CURRENT_WT" | breadth_limit |
    while IFS= read -r wt; do
      echo "=== $wt"; (cd "$wt" && git status --short | head -n 10)
    done
    ;;
  up)
...
<date> · <weekday-localized>, <time> <tz><!-- append " · 🧵 \`<CLAUDE_SESSION_NAME>\`" ONLY when non-empty; the literal "Claude session" word comes from $session_label -->
...
## 🌿 $state_detail
- $worktrees: <n> <!-- COUNT ONLY under default --scope=current (v1.8.1); paths are an explicit
     --scope=sideways expansion — see Phase 1 §"Scope boundary" and anti-pattern #29. Never
     render `<path>` entries here unless SCOPE_VERB=sideways. -->
- $untracked: <n> files
...
| **R2 Synthesize multi-tier objectives** | Decompose into **Originários (P0, causal root — *why we started at all*; PLURAL when the session absorbed an external graft, e.g. a handoff seed from a different prior session)** · Principais (P1, operator-stated top deliverable *now* — may have migrated to a derived sub-objective) · Secundários (P2, operator-authorized derivativos) · Auxiliares (P3, process-derived disciplines aplicadas). Each objective gets tasks/subtasks/steps tree. **⚠️ Anti-ceremony (per `over-engineering-circuit-breaker`/Gordian): OMIT the P0 tier entirely when `originating == primary` trivially** — a single-thread session gets no 4th-tier ceremony. P0 renders ONLY on genuine divergence (multiple causal roots OR external graft). Taxonomy inherited from `maos:goal-recovery` (SSOT) — propagated, not reinvented |
...
| **R4 Enumerate completeness gaps** | Gaps · Pendings (workflow-state) · Unasked questions (Claude should have asked) · Unanswered questions (operator AskUserQuestion pending) · Undecided decisions (open option matrices A/B/C/D) · HITL pendings (operator approval gates outstanding) |
...
<!-- P0 tier renders ONLY when originating != primary (multiple causal roots OR external graft).
     OMIT this entire ### block when originating == primary trivially (single-thread session) —
     anti-ceremony per over-engineering-circuit-breaker/Gordian. -->
### $originating · P0   <!-- omit entirely when == primary -->
\`\`\`
🌱 R1. <causal root — why we started at all> (source: <operator turn N | external graft: <session-id>>)
🌱 R2. <2nd causal root, when an external handoff was absorbed>
\`\`\`
...
| PRs criados | <n> | — |
| PRs merged | <n> | — |
| % PRs Green (CI all-pass) | <n>/<total> = <pct>% | `[<bar>]` (per §4-bar lookup; omit + diagnose if unmeasured) |
| % PR Agentic Convergence | <n>/<total> = <pct>% | `[<bar>]` (per §4-bar lookup; omit + diagnose if unmeasured) |
| % Plan execution (LLM-estimated) | <n>/<total> = <pct>% | — (LLM-estimated — NO bar per anti-pattern #30) |
| % Principais completos (LLM-estimated) | <n>/<total> = <pct>% | — (LLM-estimated — NO bar per anti-pattern #30) |
...
<!-- The human line for a measured metric ALWAYS reads:
     ...
     The `[<bar>]` string is decorative (anti-pattern #31): dropping it loses NO information.
     Rendering rules, the 11-row lookup, state cutoffs, --format=json shape and the omit-on-hole
...
## 12. $risks · <n>   <!-- NET-NEW v1.9.0 · omit when n=0 -->
...
**Determinism — the LLM never does arithmetic.** `<pct>` is already materialized by R3 in the §4 rows (`<n>/<total> = <pct>%`); the bar does not recompute it (recomputing would risk R3-vs-bar divergence). The `pct → bar` map is an **exhaustive 11-row literal lookup table** — the LLM copies one row's string; there is no fill calculation, no rounding to decide:
...
**`--format=json`** emits one object per measured metric (keys en-US, versioned with `prompt_version`; `state` is always a first-class field, `bar` is convenience never source-of-truth):
```json
...
| **3** | `AIPROVIDER_ROOT` non-empty (Claude/Cursor/Aider detected via Phase 0) | `${AIPROVIDER_ROOT}/docs/{mode}s/{YYYY-MM-DD}-{cwd-slug}.md` |
| **4** | Fallback (no repo, no AI provider) | `./{YYYY-MM-DD}-{mode}-{cwd-slug}.md` (cwd) |

Where:
- `{YYYY-MM-DD}` = current UTC date (`date -u +%Y-%m-%d`)
- `{SSID8}` = first 8 chars of `$CLAUDE_CODE_SESSION_ID` when set; **otherwise `<HHMMSS>x` timestamp suffix** (e.g., `210925x`) per Phase 0 nounset-safe fallback. Timestamp guarantees filename uniqueness on non-Claude hosts (Cursor/Copilot/Aider) where the env var is reliably unset — repeated saves same day+mode get distinct filenames without operator needing `--save-overwrite` (per Bug-2 fix v1.4.0).
...
|---|---|---|
| Claude Code (env set) | first 8 chars of UUID | Session-bound; collision only if same session re-saves same mode same day — operator opts in via `--save-overwrite` |
...
Per Bug-2 Qodo finding 2026-05-21 (PR #56 PDCA iter 1): without timestamp fallback, non-Claude hosts would always produce `{YYYY-MM-DD}_xxxxxxxx_{mode}.md` → 2nd save same day = guaranteed collision + abort. Timestamp suffix preserves the default-cascade UX cross-vendor.
...
5. **PII leak** — never hardcode named people in briefing output. Use role abstractions (`@vek-im/<team>`) or paths. Briefing is potentially shared.
...
12. **Session-name "unknown" theater** — when `$CLAUDE_SESSION_NAME` is empty (non-Claude host OR ai-title absent), do NOT render `🧵 Claude session: unknown` — silently omit the line. Rendering a literal `unknown` adds noise without information (violates `anti-theater-grounding-protocol` Layer 5 R3 hallucinated + R6 applicable). Discovered cycle-3 design 2026-05-20.
...
- **Multi-language operator (pt-BR / en-US)** → detect from CLAUDE.md / MEMORY.md; default fallback en-US with mixed-language tolerance
- **Claude session-name absent** (older Claude Code OR fresh session sem ai-title yet) → header silently omits the `🧵 Claude session` line; no `unknown` fallback (anti-theater per gotcha #12)
...
- **Concurrent parallel sessions** (operator em 2 terminais Claude Code no mesmo repo) → each session has own `CLAUDE_CODE_SESSION_ID` → briefing uses current-shell's env (no cross-contamination)
...
- **No locale env vars set** (CI / containers / sandboxed shells) → `LANG`, `LC_MESSAGES`, `LC_ALL` all unset → cascade falls through steps 1-3 to step 4 (CLAUDE.md parse) → if also absent → step 5 (en-us fallback). AAIF-safe default. No diagnostic emitted (silent success).
...
- **Repo has N sibling worktrees** (other agentic sessions/tasks sharing the repo) → default `--scope=current` briefing reports ONLY this worktree's own state + a worktree COUNT (`$worktrees <n>` in Pulse); sibling worktrees' paths/status are never scanned or rendered unless `--scope=sideways` is explicitly passed. Per anti-pattern #29.
...
| Cross-vendor | ✅ | AAIF spec; tested-via-design Claude Code; Cursor/Copilot/Aider via fallbacks |
...
| 1.0.1 | 2026-05-20 | **PATCH refinement** — adds Claude session-name field (`🧵 Claude session:` header line) per operator directive 2026-05-20. Source-of-truth: JSONL `type:"ai-title"` `.aiTitle` (rolling, `tail -1`) + `CLAUDE_CODE_SESSION_ID` env var (UUID). Probe latency <100ms empirically validated. Graceful degradation when absent (non-Claude hosts OR no ai-title yet) — silent omit, NO `unknown` literal (anti-theater per gotcha #12). 2 new anti-patterns (#12 session-name theater · #13 PII display vs persistence). 7 new edge cases (absent · cross-vendor · special-chars · >80 chars · empty string · concurrent sessions · JSONL parse-fail). Frontmatter `prompt_version` bump v1.0.0→v1.0.1 per `[C07b]`. **NOTE**: superseded by v1.1.0 in same PR (no v1.0.1 released to main as standalone). |
...
- [Anthropic claude-progress.txt pattern](https://code.claude.com/docs/en/how-claude-code-works) — session-end state file
