...
| `active_world` *(1.4.0)* | OPTIONAL | Enum `personal` \| `work` \| `unknown`, operationalizing the operator's **Two-Worlds hard boundary** (`~/.claude/rules/user-rules.md` — Vek/employer vs. C8it-Eko/personal; never mix identities/repos/credentials across them). Determined at emission time from the repo's git remote/org against the producer's OWN verified identity map (e.g. `multi-identity-git-ssh-governance.md` §0.5 domain firewall: `vek-im/*` + named work-trees ⇒ `work`; the producer's own personal org, e.g. `ekson73/*` ⇒ `personal`) — **verified, never asserted** (Best-practice #2): an unrecognized remote, a fork, or any repo that does NOT positively match one of the producer's verified identities MUST emit `unknown`, **never default to `personal`** (a downstream consumer's employer/personal split is unknowable to this producer without its own configured identity map — a silent default would assign a credential-boundary classification with no evidence). Lets a resuming agent (human OR another agent) confirm which identity/credential/attribution world it is re-entering BEFORE acting. Note the two DISTINCT "don't know" states: the field's **absence** = a pre-1.4.0 seed (this contract version didn't exist yet); the field **present with value `unknown`** = this producer tried to verify and could not — neither is ever guessed into `personal` or `work`. |
