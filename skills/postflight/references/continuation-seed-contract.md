...
| `kind` *(1.3.0)* | OPTIONAL | Producer-depth discriminator, enum `deterministic-snapshot` \| `rich-synthesis`. `deterministic-snapshot` = the PreCompact skeleton (git facts + `refs` + `resume_instructions`, no agentic synthesis); `rich-synthesis` = the P3 HANDOFF full seed. **Upgrade-only invariant**: a snapshot producer MUST NOT overwrite an existing `rich-synthesis` seed — the skeleton is strictly poorer, and `latest.json` is the RELOAD source. Absent ⇒ treat as `deterministic-snapshot` (back-compat). |
| `compact_summary` *(1.3.0)* | OPTIONAL | The harness's own PostCompact conversation summary, merged into the seed by `postflight-postcompact.sh`. Object `{present: bool, trigger, captured_at, bytes, truncated: bool, redacted: bool, text}`. `redacted:true` ⇒ a secret pattern was detected and `text` was dropped (metadata-only hard rule §Best-practices 3). `truncated:true` ⇒ `text` was size-capped. Enriches the seed **monotonically** — a reader uses it when `present`, degrades gracefully when absent. |
| `active_world` *(1.4.0)* | OPTIONAL | Enum `personal` \| `work` \| `unknown`, operationalizing the operator's **Two-Worlds hard boundary** (`~/.claude/rules/user-rules.md` — Vek/employer vs. C8it-Eko/personal; never mix identities/repos/credentials across them). Determined at emission time by `seed_active_world()` (`lib/seed-io.sh`) from the repo's `origin` remote **GitHub org** — the verified unit is the org, not a repo-name prefix, since all Vek work repos live under the SAME `vek-im` org (`multi-identity-git-ssh-governance.md` §0.5): `vek-im` ⇒ `work`; `ekson73` ⇒ `personal` — **verified, never asserted** (Best-practice #2): any other org, an unrecognized/non-GitHub remote, a fork, or a missing remote MUST emit `unknown`, **never default to `personal`** (a downstream consumer's employer/personal split is unknowable to this producer without its own configured identity map — a silent default would assign a credential-boundary classification with no evidence). Populated by every 1.4.0+ producer (`postflight-precompact.sh`, `postflight-postcompact.sh`'s fallback-skeleton path, and the P3 rich synthesis) and rendered by `reload-session.sh`. Lets a resuming agent (human OR another agent) confirm which identity/credential/attribution world it is re-entering BEFORE acting. Note the two DISTINCT "don't know" states: the field's **absence** = a pre-1.4.0 seed (every 1.4.0+ producer populates it); the field **present with value `unknown`** = a producer tried to verify and could not — neither is ever guessed into `personal` or `work`. |
...
| `plugin-scripts/governance/postflight-precompact.sh` | Producer (deterministic snapshot) | PreCompact safety-net: emits a **skeleton** seed (`params.kind = "deterministic-snapshot"`, git facts + `refs` {git,ticket,session,transcript,cwd} + `active_world` *(1.4.0, via `seed_active_world()`)* + `resume_instructions` + `data.contract_version`) to `$GIT_DIR/maos/continuation-seed.latest.json` — honest subset, never fakes the agentic synthesis. **Upgrade-only** (1.3.0): skips the write when the existing seed is already `rich-synthesis` (never clobbers the RELOAD source), under the cross-session seed lock. |
| `plugin-scripts/governance/postflight-postcompact.sh` *(1.3.0)* | Producer (compact-summary merger) | PostCompact hook: merges the harness `compact_summary` into `params.compact_summary` of the SAME seed file, under the seed lock, atomic. When enriching an EXISTING seed, backfills `params.active_world` *(1.4.0)* via `seed_active_world()` ONLY if absent (`//=`, never overwrites a value a prior producer already set); the minimal-skeleton fallback path (no seed on disk) sets it directly. Secret-scans the summary before persisting (`redacted:true` on hit) + size-caps (`truncated:true`). Never `exit 2` (fail-open); requires jq (no jq → no-op). Never touches `params.kind` (a snapshot stays a snapshot, a rich seed stays rich — it only *enriches*). |
| `plugin-scripts/governance/reload-session.sh` *(1.3.0)* | Consumer (SessionStart[compact] injector) | On `source=compact`, renders the seed (deterministic jq, fixed priority active_world→goal→dod→next_actions→gaps→compact_summary→refs→resume_instructions *(active_world added 1.4.0 — rendered FIRST, before the mission, since which identity/credential world a resuming agent is in is foundational orientation)*) into `additionalContext` (≤10k, phrased as facts) so the post-compaction agent wakes oriented. Reads `compact_summary` from the seed **file** (not stdin) → immune to the undocumented PostCompact↔SessionStart ordering. Never injects on `startup`/`clear`; always exit 0; never fabricates. |
...
| Version | Date | Change |
|---|---|---|
| 1.4.0 | 2026-08-21 | MINOR — 1 new OPTIONAL field `active_world` (enum `personal`\|`work`\|`unknown`), operationalizing the operator's `[[user-rules]]` Two-Worlds hard boundary into the continuation-seed so a resuming mind never accidentally crosses it. Sourced from the repo's git remote/org against the producer's OWN verified identity map (`multi-identity-git-ssh-governance.md` §0.5 domain firewall: `vek-im/*`+work-trees ⇒ `work`; `ekson73/*` ⇒ `personal`) — **any repo that does not positively match ⇒ `unknown`, never defaulted to `personal`** (fixed pre-merge, CodeRabbit P1: an earlier draft's "everything else ⇒ personal" would have mis-classified an unrecognized downstream consumer's repo with no evidence). No REQUIRED change, no envelope break; consumers ignore it Postel-style. Producer: `skills/postflight/SKILL.md` P3 (v0.10.1). Surfaced during a bounded coverage-check of an eko-engram process-improvement braindump (`create-agent-enhanced-braindump-prompt.md`) that also proposed `ontology.active_roles/active_personas` and an `sdp_ledger` field — both considered and **deliberately deferred** (no demonstrated failure mode; would violate the contract's own "minimal-sufficient, drop vanity detail" best-practice #5 / YAGNI) rather than added speculatively. |
| 1.3.0 | 2026-07-16 | MINOR — 2 new OPTIONAL fields + 2 new consumers (no REQUIRED change, no envelope break). Adds `kind` (producer-depth discriminator `deterministic-snapshot` \| `rich-synthesis`, carrying the **upgrade-only invariant** — a snapshot must never clobber a rich seed in `latest.json`, the RELOAD source) and `compact_summary` (the harness PostCompact conversation summary, merged monotonically, secret-scanned + size-capped). Registers `postflight-postcompact.sh` (Producer — compact-summary merger) + `reload-session.sh` (Consumer — SessionStart[compact] injector). Closes the native SAVE→RELOAD spine `PreCompact skeleton (+refs) → PostCompact merge → SessionStart[compact] inject`. The precompact producer now also emits `refs` {git,ticket,session,transcript,cwd} + `data.contract_version` and writes upgrade-only under a cross-session seed lock. Producers: `postflight-precompact.sh`, `postflight-postcompact.sh`; new lib `plugin-scripts/governance/lib/seed-io.sh` (v1.0.0 — shared lock + atomic-write + rich-detect). |
