...
| `active_world` *(1.4.0)* | OPTIONAL | Enum `personal` \| `work`, the operator's **Two-Worlds hard boundary** (`~/.claude/rules/user-rules.md` — Vek/employer vs. C8it-Eko/personal; never mix identities/repos/credentials across them). Deterministically inferred at emission time from the repo's git remote/org — `vek-im/*` (or any `vector`/`vek`/`vkl`/`vks` work tree per `multi-identity-git-ssh-governance.md` §0.5 domain firewall) ⇒ `work`; everything else (incl. `ekson73/*` community repos such as this one) ⇒ `personal` — **verified, never asserted** (Best-practice #2). Lets a resuming agent (human OR another agent) confirm which identity/credential/attribution world it is re-entering BEFORE acting, closing a real cross-world safety gap the seed did not previously carry. Absent ⇒ `unknown` (pre-1.4.0 seeds; do not guess). |
...
| Version | Date | Change |
|---|---|---|
| 1.4.0 | 2026-08-21 | MINOR — 1 new OPTIONAL field `active_world` (enum `personal`\|`work`), operationalizing the operator's `[[user-rules]]` Two-Worlds hard boundary into the continuation-seed so a resuming mind never accidentally crosses it. Deterministically sourced from the repo's git remote/org via `multi-identity-git-ssh-governance.md` §0.5 domain firewall (`vek-im/*`+work-trees ⇒ `work`; else ⇒ `personal`) — never asserted. No REQUIRED change, no envelope break; consumers ignore it Postel-style. Producer: `skills/postflight/SKILL.md` P3 (v0.10.1). Surfaced during a bounded coverage-check of an eko-engram process-improvement braindump (`create-agent-enhanced-braindump-prompt.md`) that also proposed `ontology.active_roles/active_personas` and an `sdp_ledger` field — both considered and **deliberately deferred** (no demonstrated failure mode; would violate the contract's own "minimal-sufficient, drop vanity detail" best-practice #5 / YAGNI) rather than added speculatively. |
