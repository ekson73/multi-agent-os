...
description: |
  ...
  guide (intent → right agentic-* tool + invocation), audit (read-only ASH coverage/compliance),
...
evals:
  ...
  should_not_trigger:
    ...
    - "Reimplement the ASH schema or wrap an agentic-* CLI (DRY — the tools exist)"
...
- ❌ Reimplement / wrap an `agentic-*` CLI or hook (they exist — I orient).
...
- **DoR**: an intent + read access to `bin/agentic-*`, `.claude/audit/`, `.claude/hooks/`, the schema docs.
