...
| P18 | an impediment is recorded without `reported_by` / `reported_at` (ISO 8601 + offset) — provenance would then need reconstruction from message logs, which fails exactly where a compaction-inherited claim **reads complete** (§6.0) |
