...
| P16 | the §5.1 **fetch** exited non-zero, **OR** the quarantine ref count ≠ the §5.0 destination ref count **counted with peeled `^{}` lines filtered out** (⛔ `grep -vc '\^{}$'` — `ls-remote` emits an extra peeled line per **annotated** tag that `for-each-ref` does not; without the filter this clause halts on every repo with an annotated tag). A partial fetch can exit 0, so both clauses are needed — ⛔ **an empty quarantine is `unknown`, never "no divergent refs"**. Without this, a dead fetch iterates zero refs, invokes `merge-base` zero times, and the worst-ref-class over an empty set is **vacuous** ⇒ every source ref reads *"exists only on source"* ⇒ **CLEAN CARRY on the autonomous path**. **The only fail-OPEN found in this artifact.** |
...
(Residual, honestly flagged: verified for `refs/heads` + `refs/tags` incl. annotated/peeled; **not**
verified for `refs/notes`, `refs/replace`, or a destination with a `HEAD` symref oddity.)
...
# ⛔ P16 clause 2 — COMPLETENESS: the quarantine must hold as many refs as §5.0 saw on the wire.
#    A partial fetch can exit 0, so the exit check alone is not sufficient.
# ⛔ grep -v '\^{}$' is MANDATORY: ls-remote emits an extra PEELED line per ANNOTATED tag
#    (refs/tags/x AND refs/tags/x^{}), which for-each-ref does not. Without the filter this
#    clause HALTS on every repo that has an annotated tag — a false positive on a healthy fetch.
n_wire=$(git ls-remote --heads --tags <destination> | grep -vc '\^{}$')
n_quar=$(git for-each-ref refs/rct/_dst | wc -l)
[ "$n_wire" -ne "$n_quar" ] && halt "P16: fetch under-delivered ($n_wire on wire, $n_quar quarantined)"
