...
git for-each-ref --format='%(refname)' refs/tags | sort > "$LEDGER_DIR/<slug>.src-tags-before.txt"
...
diff <(git for-each-ref --format='%(refname)' refs/tags | sort) \
     "$LEDGER_DIR/<slug>.src-tags-before.txt"        # non-empty ⇒ HALT
