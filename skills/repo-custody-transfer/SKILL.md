...
##### ⭐ The eight instances ARE a fixture spec (the actionable form of "build the runner")
...
**Prose cannot hold any of these eight** — which is precisely why eight consecutive rounds found them
...
git for-each-ref --format='%(refname) %(objectname)' "$QUAR" \
  | sed "s|^$QUAR/||" > "$LEDGER_DIR/<slug>.dst-pinned.txt"
...
"$LEDGER_DIR/<repo-slug>.dst-pinned.txt"      # DESTINATION ref→sha pinned pre-loop — P17 (§5.1)
```
...
{ "repo": "<slug>", "source": "<url>", "destination": "<url>",
  ...
  "dst_pinned_path": "<LEDGER_DIR>/<slug>.dst-pinned.txt",
