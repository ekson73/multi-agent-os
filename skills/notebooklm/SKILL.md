...
Two clients are installed. Invoke this skill to determine which one to use and how. Do **not** enable `@notebooklm-mcp` unless this skill has been invoked first and the decision guide below points to it.
...
| Scripted / CI ingestion, batch source upload, automation | `notebooklm-py` CLI | Python API; no browser needed |
...
Two accounts are configured. Always pass `--profile` explicitly; never mix sources across accounts.

| Profile | Account | Use for |
|---------|---------|---------|
| `personal` | emilson.moraes@gmail.com | Personal research, open-source work |
| `work` | emilson.moraes@vectorinf.com.br | Vek/client projects, internal docs |
...
- **Phase 3 (deferred):** `sync-to-notebooklm` skill — automated doc sync using the reserved error range; tracked as Linear VKO issues (labels: `research`, `automation`)

---

*MAOS Community Skill v1.0.0 | OS-agnostic, project-agnostic*
