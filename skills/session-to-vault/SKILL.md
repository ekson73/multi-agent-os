...
Two rows in the registry are **fixed** by construction (`markdown`/`html`/`diagram`/`notebooklm` are local;
`artifact`/`slides` are external). One is **not**: `one-pager` renders through whichever PDF producer
