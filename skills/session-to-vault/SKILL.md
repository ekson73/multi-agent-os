...
| `--scanner` | `gitleaks` | The secret-scanner the § Sanitize Gate runs over the whole staged set before any commit. Any command honouring the gate contract (receives a staged path, returns exit 0 = clean / exit 1 = hit / anything else = scan-error). Absent, unrunnable, or scan-error ⇒ treated as **unavailable** (fail-closed, same as a hit for write purposes) — but reported with an error naming the prerequisite or the failure, **never** phrased as if a secret were found. |
...
5. **On a local-sink move failure at step 3** (e.g. the note's move succeeds but a local cast's move then fails
   on an unwritable destination) — **roll back every local sink already moved in this commit** (move it back to
   staging, or delete if the destination cannot accept it) before reporting failure. Unlike an external
   publication, a local filesystem move IS reversible, so the local-sink batch stays all-or-nothing: the note
   must never end up committed with `casts` metadata pointing at a cast that was never actually written. This
...
| **Contract** | the command receives **a path to the exact bytes about to be written** and returns **exit 0 = clean · exit 1 = hit** (a real finding). Anything else (command absent, non-executable, other exit code, timeout, crash) = **scan-error, treated as unavailable** — write-blocking like a hit, but reported as a different message (see below) |
...
- exit **0** = clean · exit **1** = **hit** (a real finding) · anything else (exit ≥2, not runnable, timeout,
  crash) = **scan-error**, treated as **unavailable**. A scanner malfunction is not a credential and must never
  be reported as one — this repo's own convention draws the identical line
  (`plugin-scripts/session-fission/snapshot.sh`: `0=clean · 1=findings · else=scan-error`), and this gate follows it;
...
#    -> gate calls it with the temp path; exit 0 = clean, exit 1 = hit, anything else = scan-error (unavailable).
```
