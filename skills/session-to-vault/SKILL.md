...
or leave a reserved name (`.`, `..`); an empty or reserved slug is **rejected**, never silently used to build
`custom:{slug}.md` — write nothing under a `custom:` pattern whose slug sanitizes to empty, and derive a
deterministic non-empty fallback (e.g. from the source's own `source_session_id` short hash) before path
validation runs, so two different empty-sanitizing inputs cannot collide on the same path either.
...
| `source_params` | always | the exact `{format, style, scope, focus, context, filters, provenance, tags}` used — every output-affecting parameter this skill's own header lists (`--provenance`/`--tags` change the persisted body/frontmatter as much as `--format`/`--style` do), so a re-run is reproducible and two exports with differing omitted inputs are distinguishable |
...
3. **Marker present, but `source_session_id` OR `source_params` differ** → a **genuine collision** between two
   ...
   short, stable suffix from the run's own `source_session_id` (e.g. `{slug}-{sha256[:8]}`) and write there
   instead — report both the collision and the disambiguated path chosen, so it is discoverable, not silent.
