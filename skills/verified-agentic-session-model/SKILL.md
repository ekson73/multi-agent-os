...
- `portable_sanitized + public_only + sanitized_public_only` distribution, enforced by real URL/host parsing — loopback, RFC1918/link-local ranges, `.local`, single-label hosts, embedded credentials, and IPv4-mapped/NAT64 IPv6 forms embedding a private address are all rejected, not just an `https:`/`urn:` scheme check;
- public `https`/`urn` references with digests, `urn:` restricted to an explicit allowlisted namespace prefix;
...
`derive-child` requires the exact parent digest and a closed public request. It builds a fresh minimal child context, intent, two-world organization, topology, planned work register, nonbinding authority snapshot, empty analysis/lifecycle, and new traceability rather than cloning parent project semantics. Deterministic generation records `ai_llm.provider=none` and `model=deterministic-template`. The request's closed contract requires `inherited_material_ids` — a bounded (≤300), unique, explicit allowlist of material IDs the child may inherit. Only public inert `dna`, `template`, and `governance` records named in that list, plus the minimum public references those records require, cross the boundary; an id that does not resolve to eligible parent material fails the whole request closed (masked diagnostic, no partial child write) rather than being silently dropped. No collection is ever inherited wholesale by kind or by public label alone. Parent seed/instruction/prompt/directive content, topology, intent, references, statuses, and work history never cross the boundary regardless of the request. Fixed tool-owned neutral defaults make seed, instruction, prompt, and directive categories self-contained, and fill DNA/template/governance only when the request names no allowlisted reusable parent record for that collection. Defaults cite a dedicated public `urn:public:verified-agentic-session-model:child-defaults:v2` reference whose digest covers the fixed defaults source; each default’s `source_sha256` exactly equals that referenced digest. Every inert material is public untrusted data with `binding=false` and `effect_class=none`, and semantic validation enforces source/reference digest equality. The output has a new identity, immutable parent artifact/source/plan lineage, requested capabilities as an enforced subset, depth at most 2, no authority transfer, and no recursive spawn. A depth-2 request must omit parent-sync capability or it fails as a whole; the CLI never silently narrows it. The current external agent may validate/render the child in a later explicit invocation; the HTML never initiates it.
