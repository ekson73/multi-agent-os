...
// single source of truth for the sidecard CSP so render and verify can never drift -
// img-src tightened to 'none': no <img> tag is permitted by the passivity denylist and
// PORTABLE_CSS contains no url(data:...) anywhere, so the prior "img-src data:" allowance
// granted nothing and only widened the policy past the stated default-deny intent.
function buildCsp() {
  return `default-src 'none'; base-uri 'none'; connect-src 'none'; img-src 'none'; style-src 'sha256-${createHash("sha256").update(PORTABLE_CSS).digest("base64")}'; font-src 'none'; media-src 'none'; object-src 'none'; frame-src 'none'; worker-src 'none'; manifest-src 'none'; form-action 'none'`;
}
...
function renderPortableSidecard(model, stemOverride = undefined) {
  ...
  const edgeKey = [...new Map(model.topology.edges.map((edge) => [`${edge.role ?? "main"}\u0000${edge.variant ?? "default"}`, [edge.role ?? "main", edge.variant ?? "default"]])).values()].map(([role, variant]) => `<li><svg class=\"legend-edge-swatch\" width=\"28\" height=\"12\" aria-hidden=\"true\"><g class=\"edge-group edge-role-${role} edge-variant-${variant}\"><line class=\"svg-edge\" x1=\"1\" y1=\"6\" x2=\"27\" y2=\"6\"/></g></svg><code>${escapeHtml(`${role} / ${variant}`)}</code></li>`).join("");
  ...
  const workItemsTotal = model.plan.work_items.length;
  const workItemsDone = model.plan.work_items.filter((w) => w.lifecycle_state.state === "completed").length;
  const workItemsBlocked = model.plan.work_items.filter((w) => w.lifecycle_state.state === "blocked").length;
  const workItemsInProgress = model.plan.work_items.filter((w) => w.lifecycle_state.state === "started").length;
  const workItemsPct = workItemsTotal ? Math.round((workItemsDone / workItemsTotal) * 100) : 0;
  // union DoD + acceptance_criteria - PO found the prior version undercounted (2 vs the
  // true 5) by only scanning one of the two criteria registers that both feed the same
...
<header class=\"hero\"><h1 translate=\"no\">${escapeHtml(model.metadata.title)}</h1><p class=\"subtitle\">${escapeHtml(model.metadata.scope)}</p><div class=\"snapshot-strip\"><span class=\"status state-planned\"><span aria-hidden=\"true\">○</span>Snapshot · Not Live</span><span>Generated externally</span><span class=\"trust\">Portable trust: CONSISTENT_UNTRUSTED</span><span class=\"trust-gloss\">(a standing risk/trust classification for this snapshot - not a report of an active incident)</span></div><p class=\"why-matters\"><strong>Why this matters:</strong> ${escapeHtml(model.intent.motivations[0])}</p><p class=\"goal\"><strong>Current goal:</strong> ${escapeHtml(model.intent.goal)}</p><p class=\"tldr\"><strong>10-second read:</strong> ${escapeHtml(execStatusWord)} (${escapeHtml(derived.next.disposition)}) · ${escapeHtml(String(workItemsDone))} of ${escapeHtml(String(workItemsTotal))} work items complete (${escapeHtml(String(workItemsPct))}%) · ${escapeHtml(String(workItemsBlocked))} blocked · ${escapeHtml(String(workItemsInProgress))} in progress · decision needed: ${escapeHtml(decisionNeeded)} · next: ${escapeHtml(truncateAtWord(nextAction.what, 90))}</p><p class=\"tldr\" lang=\"pt-BR\"><strong>Leitura de 10 segundos:</strong> ${escapeHtml(execStatusWord === "On track" ? "Em dia" : execStatusWord === "Blocked" ? "Bloqueado" : "Em risco")} (${escapeHtml(derived.next.disposition)}) · ${escapeHtml(String(workItemsDone))} de ${escapeHtml(String(workItemsTotal))} itens de trabalho completos (${escapeHtml(String(workItemsPct))}%) · ${escapeHtml(String(workItemsBlocked))} bloqueados · ${escapeHtml(String(workItemsInProgress))} em andamento · (inglês acima é o idioma padrão do documento; o resumo em pt-BR usa os mesmos dados)</p><p class=\"meta\">As of <time datetime=\"${escapeHtml(identity.chronological.as_of)}\">${escapeHtml(identity.chronological.as_of)}</time> · <span class=\"artifact-id mono\" translate=\"no\">${escapeHtml(artifactId(model))}</span></p></header>
<div class=\"grid\">
<section class=\"panel wide priority\" id=\"next\"><h2>Current Disposition</h2><p><strong>${escapeHtml(derived.next.disposition)}</strong>${derived.next.task_ref ? ` · <span class=\"mono\">${escapeHtml(derived.next.task_ref)}</span>` : ""}</p><dl class=\"next-action\"><dt>What</dt><dd>${escapeHtml(nextAction.what)}</dd><dt>Who</dt><dd>${escapeHtml(nextAction.who)}</dd><dt>When</dt><dd>${escapeHtml(nextAction.when)}</dd><dt>Why</dt><dd>${escapeHtml(nextAction.why)}</dd><dt>Where</dt><dd>${escapeHtml(nextAction.where)}</dd><dt>How</dt><dd>${escapeHtml(nextAction.how)}</dd></dl><details class=\"register\"><summary>Work Register · ${model.plan.work_items.length} Items</summary><ul>${work || "<li>None</li>"}</ul></details><details class=\"register\"><summary>Why this exists · Motivations</summary><ul>${model.intent.motivations.map((m) => `<li>${escapeHtml(m)}</li>`).join("")}</ul></details></section>
...
<section class=\"panel\" id=\"traceability\"><h2>Traceability</h2><dl><dt>Author</dt><dd>${escapeHtml(trace.author.public_id)} · ${escapeHtml(trace.author.attribution)}</dd><dt>Generator</dt><dd>${escapeHtml(receipt.generator.name)} v${escapeHtml(receipt.generator.version)} (${escapeHtml(receipt.generator.runtime)})</dd><dt>Harness</dt><dd>${escapeHtml(trace.ai_harness.name)} ${escapeHtml(trace.ai_harness.version)}</dd><dt>Model</dt><dd>${escapeHtml(trace.ai_llm.provider)} · ${escapeHtml(trace.ai_llm.model)}${trace.ai_llm.revision ? ` · ${escapeHtml(trace.ai_llm.revision)}` : ""}</dd><dt>Created</dt><dd>${escapeHtml(trace.created_at)}</dd><dt>Updated</dt><dd>${escapeHtml(trace.updated_at)}</dd><dt>SemVer</dt><dd>${escapeHtml(trace.semantic_version)}</dd><dt>Git client</dt><dd>${escapeHtml(trace.git.client_version)}</dd><dt>Git tracking</dt><dd>${escapeHtml(trace.git.tracking_state)} · ${escapeHtml(trace.git.repository_visibility)}</dd><dt>Repository</dt><dd class=\"mono\">${escapeHtml(trace.git.repository_uri || "undisclosed")}</dd><dt>Ref</dt><dd class=\"mono\">${escapeHtml(trace.git.ref_name || "undisclosed")}</dd><dt>Commit</dt><dd class=\"mono\">${escapeHtml(trace.git.commit_sha || "undisclosed")}</dd><dt>Tree</dt><dd class=\"mono\">${escapeHtml(trace.git.tree_sha || "undisclosed")}</dd></dl></section>
...
<section class=\"panel\" id=\"materials\"><h2>Inert seed, DNA, template, instruction, prompt, directive, governance material</h2><ul>${materials || "<li>None recorded</li>"}</ul><details><summary>Machine capsule contract</summary><p>Exactly two base64url canonical-JSON blocks follow. They are inert data for an external reader; artifact content is nonbinding and has no browser effects.</p></details></section>
  ...
  // Chromium's file:// loader serves local HTML as application/octet-stream (no HTTP
  // Content-Type header), so its own charset SNIFFER can override an explicit
  // <meta charset="utf-8"> -- observed and reproduced: document.characterSet resolved to
  // windows-1252 despite byte-correct UTF-8 content, turning every state glyph (checkmarks,
  // bullets, arrows) into mojibake on the artifact's primary, stated distribution path
  // (opening the file directly). A UTF-8 BOM (EF BB BF) is the standard, reliable
  // countermeasure: it forces the encoding unambiguously before any sniffing heuristic runs.
  const htmlBytes = Buffer.concat([Buffer.from([0xef, 0xbb, 0xbf]), Buffer.from(html)]);
...
// Word-boundary truncation for prose (unlike truncateToUnits, which intentionally cuts
// mid-character for fixed-width SVG labels elsewhere) -- a summary line that visibly
