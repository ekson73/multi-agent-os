...
| **Semgrep** | **often no committed rule file** — commonly CI-registry-driven (`semgrep --config p/ci`); a local `.semgrep.yml` / `.semgrep.yaml` is frequently absent | SAST findings | ⚠️ CI-tuning (not a committed rule file when registry-driven) | narrowest = a comment **in the flagged file's own language syntax** containing `nosemgrep` + rationale (e.g. `// nosemgrep: <rule-id>` for JS/TS/Java, `# nosemgrep: <rule-id>` for Python/YAML/shell — a `#` comment is a no-op in `//`-comment languages); else tune the CI `--config` / severity. ⛔ making the gate **non-blocking** (e.g. `\|\| true`) is a **suppression anti-pattern**, NOT a teaching form — HITL-only, with alerting + expiry, never silent. ⚠️ security-class → verify-gated, never blanket-disable |
