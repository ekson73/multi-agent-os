...
- **One per-seat parent, one unit worktree per writer.** Before launch, create an empty parent per writing
  ...
  merges only that SHA (for GitHub, `gh pr merge <number|url> --<merge|squash|rebase per the target repo's policy> --match-head-commit <sha>`); any other SHA needs a new
...
- **Project-scoped config does not follow a desk seat (mandatory pre-launch step).** With the cwd outside the
  ...
  2. **MCP servers:** project them, do not only approve them. Copy the reviewed server definitions from the
     ...
     server runs outside any bash sandbox with the operator's credentials. One whose executable or
     dependencies sit in a seat-writable location is an unconditional launch stop. Under the sandbox path, one
     with filesystem, credential or network capability is a launch stop unless the credential has been removed
     at its source ([`sandboxed-seats.md`](./sandboxed-seats.md) §0).
