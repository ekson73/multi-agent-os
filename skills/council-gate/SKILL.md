...
- emits an `AUTHORIZE_CONSULTATIVE` **verdict** for a qualifying safe-class action (the unarmed/consultative path; the armed path emits `AUTHORIZE_EXECUTED`, §6.3),
...
- **GDPR Art.22 / LGPD Art.20** (a **conservative deny-by-default operating assumption, NOT a legal ruling** — the gate informs + denies, it does not give legal advice; cf. `[[lgpd-residency-not-localization]]` §3 + `[C17]` §2, legal applicability = operator/DPO/counsel domain) — any action that constitutes/determines/executes a decision that **could** carry a **legal or similarly-significant effect on a natural person** (credit/eligibility · pricing · hiring/termination · benefit/access grant-or-deny · account-suspension · consequential content-moderation) → **HARD → mandatory-HITL**. *Why the council can't self-authorize (design rationale):* the gate **treats** a converged council + red-team as *still* solely-automated — the SCHUFA CJEU C-634/21 rubber-stamp doctrine (only *meaningful* human involvement launders a solely-automated decision) is **why** it conservatively assumes it is **not** the Art.22(3) safeguard. *What it does NOT decide:* whether Art.22 **legally applies** (EU/data-subject nexus · actual effect · what "meaningful human involvement" satisfies it · an Art.22(2) exception [consent · contract · law]) is a **jurisdiction/fact-specific determination for the operator's DPO/legal** — surfaced by the `data-privacy-officer` seat, **never established by the council at runtime**. Adopts GDPR Art.22 as a **conservative internal floor** (LGPD Art.20 §3's human-reviewer was vetoed) **without asserting which regime governs**.
- Mechanically: honor the existing `prevent-main-commit`/`enforce-worktree` hooks + `gitleaks` + `pr-review-protocol` §2.6.1 R1-R6. Unknown ⇒ **BLOCK** (fail-safe). A jailbroken council never reaches Layer 2 for a deny-set action, because Layer 1 fails on it independently. *(NB: `bin/convergence-guard` is **not** a Layer-1 deny-set enforcer — it is the Layer-2/P4 convergence gate, §5.2. A Layer-1 deny-set match is `HITL_HARD_BLOCK`; a Layer-2 `convergence-guard` REFUSE is `HITL_LOW_CONFIDENCE` — audit records which layer produced the verdict.)*
...
**Seats by domain** (compose existing agents — the full SDLC roster; *maos-native* = always loadable, *cross-plugin* = requires that plugin enabled, verify per Mente Tomé; authoritative SSOT = `multi-agent-os/agents/README.md` "Role Coverage Map"): PO/PM/SM/BA → `agile-product-lead` (**composite — 4 roles**) · SA → `architecture:architect` (cross-plugin ✓enabled) · QA → `qa-validator` (fuller alt `architecture:qa-engineer`) · TESTER/SDET → `architecture:test-generator` (cross-plugin ✓enabled) · UX/UI/Designer → `ux-design:ui-designer`+`ux-design:ux-optimizer` (**cross-plugin — ⚠ requires `ux-design` enabled, NOT active this session**) · dev-fe → `react-frontend-engineer`/`angular-frontend-engineer` · dev-be → `quarkus-backend-engineer` · db(Supabase/Postgres) → `supabase-engineer` · devops → `gitops-engineer` (broader alt `cloud-infrastructure:deployment-engineer`/`cloud-architect` ✓enabled) · devsecops → `governance-auditor` (alts `architecture:security-reviewer`/`code-modernization:security-auditor`) · dpo → `data-privacy-officer` · prompt/context-eng → `prompt-context-engineer` (**composite — prompt+context+harness**). **Gaps (documented, NOT forged — YAGNI):** Neon · generic-Postgres-DBA · Aurora-DBA · broad-ai-eng · **loop-eng** (owner of the rule §5.5.1 wiring) → rule Deferred D4.

**Seat-selection (stakes-scaled, anti-over-council — Gordian):** seat only the advisors the domain implicates, scaled by `--stakes` — `trivial|low` → 1–2 relevant seats + light convergence; `high` → full domain board + `persona-pipeline` deep-pass + §5.2.5 red-team. This **parameterizes** the existing `persona-pipeline` risk-profile depth-scaling (never a full 13-seat board for a typo); `--seats` overrides. **Privacy/LGPD/GDPR is a STANDING lens (NOT stakes-gated):** the §5.1 Art.22/Art.20 deny-set check + the §4 privacy interrogation lens + the §5.2.5 red-team privacy hunt run on **every** invocation regardless of `--stakes` (an Art.22 legal/significant-effect decision is never legitimately "low-stakes"); the `data-privacy-officer` seat is auto-added whenever the action plausibly touches personal data or an automated decision about a person. Stakes scales *council depth*, never *whether privacy is evaluated*. **Autonomy note (honest):** a broader *competent* roster widens the safe-class the gate can autonomously clear ⇒ fewer HITL fallbacks — *bounded by unchanged safety* (still Layer-1-cleared, P3 ≥ 0.90, red-team-survived; never touches the deny-set) + watched by Metron authorize-rate (§9, so a wider roster ≠ rubber-stamp). The *realized* HITL reduction still needs the rule §5.5.1 D1/D2 wiring (operator-gated, `needs_hitl`) — this roster *prepares* it.

Convergence gate = `maos:convergence-engine` → **`bin/convergence-guard`** (deterministic ALLOW/REFUSE — never model-judged; a REFUSE here is a **Layer-2 non-convergence** → `HITL_LOW_CONFIDENCE` (P4 unmet), **distinct** from a Layer-1 deny-set `HITL_HARD_BLOCK` — `decision-capture` records which layer). **Independent verifier** (verify-stage ≠ the generator — Explainability-Paradox guard): the verifier must NOT be persuaded by the generator's own justification. Uplift = `maos:cascade-resolver` (REFINE, economic-stop `n*≤3-4`).
...
| 1.0.0 (PDCA-revised · Round-2c · CodeRabbit) | 2026-07-14 | **Third PDCA cycle** — CodeRabbit re-reviewed `05c4277` (actionable count 5→2, healthy convergence). Both addressed: **§6.3 verdict↔predicate mapping** made explicit (CR Major, functional-correctness) — `AUTHORIZE_EXECUTED`⟺P1–P5 · `AUTHORIZE_CONSULTATIVE`⟺P1–P4∧¬P5 · `HITL_*`⟺safe-class P1–P4 fails; the **safe class is P1–P4**, P5(armed) is the only execute/consult difference; MD022/MD031 blank lines around the §6 predicate fence + below the §6.3 heading (CR Minor). (amazon-q "future date 2026-07-14" = same clock-cutoff false-positive, today IS 2026-07-14 — not applied.) No version bump (pre-merge). |
