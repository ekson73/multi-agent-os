...
1. **Layer 1 — deterministic deny-set** (out-of-band, evaluated FIRST, independent of confidence): secrets · production PII · irreversible/prod · merge→main/prod · push-force · `--no-verify` · cross-org · $$$ · critical-infra/CI · `[C17]` §2 HUMAN_DOMAIN · **solely-automated decisions with legal/significant effect on a natural person (GDPR Art.22 / LGPD Art.20 — the council is *still* solely-automated, so it can't be the human safeguard)**. Match → **HARD BLOCK → HITL**. The council can never open this.
2. **Layer 2 — democratic council** (only within the cleared band): role-advisors seated by domain across the full SDLC (PO/PM/SM/BA · SA · QA · TESTER · UX · dev-fe/be · db · devops · devsecops · dpo · prompt/context-eng — composites + cross-plugin agents, stakes-scaled) converge (verifier > generator, independent), then an **independent red-team** tries to refute the safe-class classification (default-to-refuted). On convergence **+ red-team-survived** + `autonomy_score ≥ 0.90` + reversible + **armed**, the gate **authorizes in place of the human** (executes + `decision-capture` audit).
